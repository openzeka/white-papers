...
description: >-
  200G breakout'lu MikroTik CRS804 üzerinden 8 NVIDIA DGX Spark node'u ile switch
  tabanlı AI cluster kurulumu: management ve compute ağları, RoCEv2/RDMA,
  sparkrun ve NAS.
...
7. [CRS804 MikroTik Switch Yapılandırması](#crs804-mikrotik-switch-yapılandırması)
...
Bu doküman, 8 NVIDIA DGX Spark node'undan oluşan switch tabanlı bir AI cluster'ın kurulum ve konfigürasyon adımlarını baştan sona anlatmaktadır. Cluster, dağıtık AI iş yüklerini ve model çalıştırmayı yönetmek için **sparkrun** araç setini kullanır.
...
| **Bileşen**         | **Açıklama**                                                 |
| ------------------- | ------------------------------------------------------------ |
| **DGX Spark × 8**   | Her birinde ConnectX-7 200GbE QSFP56 port bulunur            |
| **MikroTik CRS312** | 10GbE management ağı switch'i; NAS bonding portlarını da içerir |
| **MikroTik CRS804** | 400G QSFP-DD compute ağı switch'i; breakout ile 8×200G sağlar |
...
- 8× NVIDIA DGX Spark sistemi

- 1× MikroTik CRS312-4C+8XS switch (management ağı)

- 1× MikroTik CRS804 switch (compute ağı)

- 1× ASUSTOR AS6808T NAS (8× HDD)

- 4× QSFP-DD → 2× QSFP56 pasif breakout kablo (compute ağı)
...
Aşağıdaki işlemler sekiz Spark sistemi üzerinde de uygulanmalıdır.
...
5.  Aşağıdaki bilgileri girin:
    ...
    - Address: 192.168.1.x (her Spark için farklı ve kendi ağınıza uygun olarak)
...
İnternet erişimi varsa 10GbE management bağlantısı hazırdır. Diğer yedi Spark üzerinde de aynı adımları tekrarlayın ve her birine farklı bir IP adresi atayın.
...
```bash
ping -c 4 192.168.1.x #Diğer Spark'ların adresleri
```
...
DGX Spark 8-Node AI Cluster'da her Spark, ConnectX-7 QSFP

portu üzerinden 200GbE hızında MikroTik CRS804 switch'e bağlanır. CRS804'ün 400G QSFP-DD portları, pasif breakout kablolar ile ikişer 200G bağlantıya ayrılır.

**Kablo Planı**

| **CRS804 Port** | **Port Hızı** | **Breakout**    | **Bağlanan Spark** |
| --------------- | ------------- | --------------- | ------------------ |
...
1. İlk breakout kablosunun QSFP-DD ucunu CRS804'ün 1 numaralı QSFP-DD portuna takın. Kablonun her iki ucundaki kilitleme mandalının tam oturduğundan emin olun.
...
3. Diğer 3 adet kabloyu da aynı şekilde Switch ve Spark'ları bağlayacak şekilde takın

4. CRS804 switch'in gücünü açın.
...
CRS804'ün dört QSFP-DD fiziksel portu her biri varsayılan olarak 400G'dir. Her 400G port, pasif breakout kablo ile iki adet 200G bağlantıya ayrılır. QSFP-DD portları 8 alt arayüze sahiptir; 2×200G modunda 200G ana arayüzler -1 ve -5'tir. Diğer alt arayüzler (-2, -3, -4, -6, -7, -8) kapatılmaz, etkin kalır ancak yapılandırma gerektirmez.
...
Tüm Spark sistemlerinde aynı kullanıcı adı oluşturulmalıdır. Bu dokümanda nvidia kullanıcı adı kullanılacaktır. Aşağıdaki komutlar sekiz Spark sistemi üzerinde de çalıştırılır:
...
Bu adımda, compute ağının doğru çalıştığını ve RoCEv2 üzerinden RDMA iletişiminin beklendiği gibi performans verdiğini doğrulayacağız. Testler iki Spark arasında yapılır; 8 node için çapraz testler benzer şekilde tekrarlanabilir.

**IP Ataması Referansı**

sparkrun wizard tarafından ilk iki spark için CX7 arayüzlerine atanan IP adresleri aşağıda göründüğü gibidir, sizin senaryoda bu adresler yerine kendi adreslerinizi kullanmanız gerekir:
...
Bu testte glm-5.2-nvfp4 modeli kullanılır. Model, 8 node üzerinde tensor parallelism ile çalıştırılır. Bu model için özel container imajı ve recipe YAML dosyası OpenZeka tarafından hazırlanmıştır.
...
Daha sonra etiketini düzenleyin:

```bash
docker tag registry.cordata.ai/spark-cluster/vllm-zatz-dcp:probe vllm-zatz-dcp:probe
```

İmaj ayarlandıktan sonra recipe YAML dosyası ile birlikte sparkrun üzerinden inference iş yükü çalıştırılabilir.
...
# --- Degiskenler burada TEK yerde; command icinde suslu-parantezle kullanilir ---
defaults:
  port: 8210
  host: 0.0.0.0
  tensor_parallel: 8
...
| Concurrency | Ortalama TTFT | Ortalama ITL |  Ortalama TPS | Ortalama Gecikme | p90 Gecikme |
| ----------: | ------------: | -----------: | ------------: | ---------------: | ----------: |
...
Test sonucunda, modelin 8 node üzerinde dağıtık olarak başarıyla çalıştığı ve çok node'lu inference altyapısının uçtan uca doğrulandığı görüldü. Concurrency arttıkça TTFT ve toplam gecikme değerlerinin arttığı, kullanıcı başına düşen token üretim hızının ise azaldığı gözlemlendi.
...
**CRS804 Breakout Portları Link Up Olmuyor**
