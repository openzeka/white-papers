...
|---|---|---|---|---|---|
| VRAM requirement | Very high (~16× params) | High (~12–14× params) | Medium (~2–3× params) | Low (~0.6–1× params) | Model + KV cache |
...
- **LoRA / QLoRA fine-tune:** The practical way to adapt an existing model to your own data. It does not train the whole model, only small additional layers. **Memory accounting:** In LoRA, base weights are frozen, so only FP16 weights (~2× params) + gradient-checkpointed activations + small adapter gradients/Adam states contribute → **~2–3× params**; in QLoRA, base weights are quantized to 4-bit NF4 (~0.5× params), plus activations and small adapter overhead → **~0.6–1× params**. Fine-tuning a 70B model even with a single **DGX Spark** is possible (with QLoRA — in FP16-based LoRA, the 70B weights alone are ~140 GB, so they do not fit in 128 GB). (Open-source tools: see 6.F.)
